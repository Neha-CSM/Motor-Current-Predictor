...
TELEMETRY_COLUMNS = [
    ...
    'timestamp', 'orientation.x', 'orientation.y', 'orientation.z', 'orientation.w',
    'angular_velocity.x', 'angular_velocity.y', 'angular_velocity.z',
    'linear_acceleration.x', 'linear_acceleration.y', 'linear_acceleration.z',
    'enc1', 'enc2', 'enc3', 'enc4', 'vel1', 'vel2', 'vel3', 'vel4',
...
def run_motor_analysis(df, target_name):
   ...
   #Filter for anomalies
    anomalies_only = work_df[iso_preds == -1].copy()
    anomaly_count = len(anomalies_only)
    clean_count = len(work_df) - anomaly_count
    ...
    formatted_df = display_df.to_string(
        index=False, 
        justify='center',
        float_format=lambda x: f"{x:12.6f}", 
        col_space=14 
    )

    with open(f"{target_name}_detected_anomalies.csv", 'w') as f:
        f.write(formatted_df)

    # Keep clean data for model training
    ...
    speedup, xgb_preds_bench, onnx_preds_bench = benchmark_models(model, session, X_test, input_name)
    
    #determine overfitting or underfitting
    total = len(residuals)
    over_p = np.sum(residuals < 0) / total
    under_p = np.sum(residuals > 0) /total
...
def benchmark_models(model, session, X_test, input_name):
    n_runs=20
    batch_multiplier=10
    ...
    speedup = f"{xgb_median/onnx_median:.2f}x"
